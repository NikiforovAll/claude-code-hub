...
Claude Code Hub — a unified launcher that combines multiple Claude Code tools (Marketplace + Kanban + Cost) into a single chromeless PWA via iframes and git submodules.
...
CLI flags: `--port <n>`, `--marketplace-port <n>`, `--kanban-port <n>`, `--cost-port <n>`, `--open`

## Architecture

**Hub server** (`server.js`) spawns three child processes — marketplace, kanban, and cost — passing `CLAUDE_HUB=1` and `HUB_URL` env vars. It parses their stdout to detect actual ports (handles fallback when default ports are busy) and exposes `GET /api/config` returning the live app URLs.
...
After cloning: `git submodule update --init` then `npm install` in root, `marketplace/`, `cck/`, and `cost/`.
